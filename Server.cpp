...
void Server::ProcessClientRequests(int client_socket) {
    char buffer[1024] = {0};
    while (true) {
        memset(buffer, 0, 1024);  // memset seteaza buffer-ul pe 0 , size = 1024
        ssize_t valread = recv(client_socket , buffer, 1024, 0);  // fac metoda de read
        if (valread <= 0) {  // daca nu primeste nimic sa incerce din nou
            continue;  // opreste executia si incepe o noua bucla
        }
        string message(buffer);  // pun string ul intr-un buffer si apoi fac split pe el
        int count = 0;
        for (auto &elem : message) {  // metoda de split
            if (elem == ',') {
            count++;
            }
        }
        ...
        for (int i = 0; i <= count; i++) {  // am pus <= count ca imi acceseze si ultimul token si sa mearga pana la count
            if (pch != NULL) {
                string tokenstr(pch);
        ...
        if (tokens[0] == "InsertProp") {  // imi dau seama ce metoda trebuie sa apelez din server, sa accesez baza de date
            Server::InsertProperties(client_socket, tokens[1], tokens[2], tokens[3]);  // procesare mesaj
...
void Server::DisplayProperties(int server_socket, string propID) {
    ...
    if (!qstate) {
        ...
        while ((row = mysql_fetch_row(res))) {
            string r0, r1, r2, r3;
            r0 += row[0];  // transformare char in string
            r1 += row[1];
            r2 += row[2];
            r3 += row[3];
