...
MenuBar::MenuBar(int server_socket, QWidget *parent) : QMainWindow(parent) {
  ...
  QAction *stockSimulation = new QAction("&Stock Simulation", this);
  ...
  connect(stockSimulation, &QAction::triggered, this, &MenuBar::showSimulation);
  ...
  detailMenu->addAction(stockSimulation);
...
void MenuBar::showSimulation(bool checked) {
  stockSimulation = new StockSimulation(server_socket);
  stockSimulation->show();
}